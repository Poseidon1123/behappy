...
Backtester v1 is an execution-cost-aware baseline, not proof of profitability. If you run it over bars that overlap the model's training period, the result is partly in-sample and can be optimistic. Do not use that result to justify live trading. The next validation step should use walk-forward retraining/testing so every simulated trade is generated by a model trained only on earlier data.
