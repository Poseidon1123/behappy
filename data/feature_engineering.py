...
def _build_h1_context(m15: pd.DataFrame) -> pd.DataFrame:
    ...
    h1 = (
        ...
        .dropna()
        .reset_index()
        .rename(columns={"m15_close_time": "h1_close_time"})
    )
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    high20 = high.rolling(20).max()
    low20 = low.rolling(20).min()
    out["range_pos20"] = (close - low20) / (high20 - low20).replace(0.0, np.nan)
    ...
    if "spread" in out.columns:
        spread = out["spread"].astype(float)
        spread_median = spread.rolling(50).median()
        out["spread_relative"] = spread / spread_median.replace(0.0, np.nan)
    else:
    ...
    out["m15_h1_bull_alignment"] = ((out["ema_diff"] > 0) & (out["h1_trend_up"] > 0)).astype(float)
    out["m15_h1_bear_alignment"] = ((out["ema_diff"] < 0) & (out["h1_trend_down"] > 0)).astype(float)
