...
class BrokerUnavailableError(TransientMT5Error):
    """The terminal cannot currently provide broker data or accept requests."""
...
def _send_checked(mt5: Any, request: dict[str, Any]) -> Any:
    ...
    if int(result.retcode) not in accepted:
        if int(result.retcode) == int(getattr(mt5, "TRADE_RETCODE_TIMEOUT", 10012)):
            raise OrderStatusUnknownError(
                f"order_send timed out: retcode={result.retcode}, comment={result.comment}"
            )
        if int(result.retcode) in {
            int(getattr(mt5, "TRADE_RETCODE_CONNECTION", 10031)),
            int(getattr(mt5, "TRADE_RETCODE_PRICE_OFF", 10021)),
        }:
            raise BrokerUnavailableError(
                f"order_send unavailable: retcode={result.retcode}, comment={result.comment}"
            )
...
def open_demo_position(
    ...
    *,
    ...
    if info is None or tick is None:
        raise BrokerUnavailableError(f"Could not read symbol/tick for {symbol}: {mt5.last_error()}")
...
def close_demo_position(mt5: Any, position: Any, deviation_points: int) -> Any:
    ...
    if tick is None or info is None:
        raise BrokerUnavailableError(f"Could not read symbol/tick for {position.symbol}: {mt5.last_error()}")
