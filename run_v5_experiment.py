...
from backtest.meta_labeling_v5 import run_meta_labeling_walk_forward_v5
from data.snapshot import load_snapshot


def _arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run causal drift-gate v5 on an immutable dataset")
...
def main() -> None:
    ...
    print("V5 CAUSAL DRIFT EXPERIMENT")
    ...
    baseline, v41, v42, buy_only, v5, folds, meta_training, drift_diagnostics, summary = run_meta_labeling_walk_forward_v5(
        data,
        cfg,
    ...
    out = Path("reports") / f"v5_{manifest['sha256'][:12]}"
